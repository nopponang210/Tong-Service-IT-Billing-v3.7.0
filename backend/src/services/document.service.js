...
async function insertSignatureSnapshot(client, documentId, settings) {
    if (!settings.saved_signature_url) return;
    await client.query(
...
async function createDocument({ body, userId }) {
    ...
    try {
        ...
        await insertSignatureSnapshot(client, document.id, settings);
