...
function calculateDocumentTotals({
    ...
    let withholdingRate = estimate.rate;
    let withholdingBase = estimate.base;
    let withholdingAmount = estimate.amount;
    ...
    if (documentType === 'RC') {
        ...
        if (!enabled) {
            withholdingRate = decimal(0);
            withholdingBase = decimal(0);
            withholdingAmount = decimal(0);
        } else {
