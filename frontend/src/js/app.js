...
function updateReceiptPaymentVisibility({ applyDefaults = false } = {}) {
  ...
  const box = $('#receipt-payment-box');
  box.classList.toggle('hidden', !isReceipt);
  if (!isReceipt) return;
...
async function openDocumentModal(documentId = null) {
  ...
  if (state.editingDocumentId) {
    ...
    $('#doc-receipt-withholding-enabled').checked = Number(doc.withholding_amount || 0) > 0;
...
function updateDocumentPreview() {
  ...
  if (product + service + other === 0) {
    const sourceTotal = $$('#source-documents-list input:checked')
  ...
  if ($('#doc-type').value === 'RC') {
    ...
    && grandTotal > Number(customer.withholding_threshold || 0)
    && customer.withholding_basis !== 'none'
  ) {
    const base = customer.withholding_basis === 'service'
      ? Math.max(service - (Number($('#doc-discount').value) || 0), 0)
      : grandTotal;
    withholding = base * Number(customer.withholding_rate || 0) / 100;
  }
...
async function loadSourceDocuments() {
  ...
  box.classList.remove('hidden');
  list.innerHTML = result.data.map((d) => `<label class="source-option"><input type="checkbox" value="${d.id}" data-total="${Number(d.grand_total || 0)}"> <strong>${escapeHtml(d.document_number)}</strong> · ${DOC_LABELS[d.document_type]} · ${dateThai(d.document_date)} · ${money(d.grand_total)}</label>`).join('');
  $$('input', list).forEach((input) => input.addEventListener('change', updateDocumentPreview));
}
