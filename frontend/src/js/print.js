...
function summaryRows(doc) {
  ...
  if (Number(doc.withholding_amount) > 0) {
    const withholdingLabel = doc.document_type === 'RC'
      ? `หัก ณ ที่จ่ายจริง ${Number(doc.withholding_rate)}%`
      : `ประมาณการหัก ณ ที่จ่าย ${Number(doc.withholding_rate)}%`;
    rows.push([
      withholdingLabel,
  ...
  rows.push([
    doc.document_type === 'RC'
      ...
      : ['IN', 'BN'].includes(doc.document_type)
        ? 'ยอดที่คาดว่าจะชำระสุทธิ'
        : 'รวมสุทธิ',
    doc.net_total,
  ]);
...
function signatures(doc) {
  ...
  const signatureSource = signatureSnapshot || doc.settings.saved_signature_url;
  const savedSignature = signatureSource ? `<img class="signature-image" src="${escapeHtml(signatureSource)}" alt="ลายเซ็น" referrerpolicy="no-referrer" onerror="this.style.display='none'">` : '<div></div>';
